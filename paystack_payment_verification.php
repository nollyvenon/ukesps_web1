...
// 		$payment = $clientOperation->paystack_payment($course_prov_code, $_GET['reference'], $_GET['trxref'], '1', $amount, $_SESSION['couprov_email'], $_SESSION['unique_id'], $payment_category, $currency);
// 		//redirect to a thank you page
// 		header('Location: paystack_thank_you.php');
// 	} else {
// 		$payment = $clientOperation->paystack_payment($course_prov_code, $_GET['reference'], $_GET['trxref'], '2', $amount, $_SESSION['couprov_email'], $_SESSION['unique_id'], $payment_category, $currency);
...
if ('success' == $tranx->data->status) {
	...
	foreach ($cart_items as $item) {
		...
		$payment = $clientOperation->paystack_payment($_SESSION['client_unique_code'], $_GET['reference'], $_GET['trxref'], '1', $cart_amount, $_SESSION['client_email'], $_SESSION['unique_id'], $payment_category, 'NGN');
	...
	$payment = $clientOperation->paystack_payment($_SESSION['client_unique_code'], $_GET['reference'], $_GET['trxref'], '1', $amount, $_SESSION['client_email'], $_SESSION['unique_id'], $payment_category, 'NGN');
	//mysql_query("DELETE FROM carrrt WHERE code='$unique'");
	header('Location: paystack_error.php');
}
