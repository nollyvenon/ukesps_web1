...
if ($_GET['token']) {
  $user = $zenta_operation->get_user_reset_token($_GET['token']);
  if (!$user) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['recaptcha_response'])) {
  ...
  if ($recaptcha->score >= 0.5) {
    ...
    if ($password !== $confirm_password) {
      ...
      $result = $zenta_operation->change_user_password($user['user_code'], $password);
      if ($result) {
        $message_success =  "<b>Congratulations!</b><br>Password Reset Successful. ";
        $message_success .= '<br>You can now <a href= ' . SITE_URL . '/login>Login</a> with your new password OR <br> <a href= ' . SITE_URL . '/login>Click Here</a>';
      } else {
