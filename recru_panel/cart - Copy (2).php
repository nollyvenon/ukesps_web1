...
if ($_POST['proceed']){
    ...
    for($i=0; $i<$max; $i++) { 
        ...
        $total_price = $_POST['total_price'];        
        $total_price = $_POST['total_price'];
        $recruit_object->add_to_cart($plan_id, $amount, $quantity, $OrderID, $unique);
						...
						<tbody>	
							...
							<tr  class="cart_item">
								...
								<td class="product-price">
                                    <span class="amount txtCal price"><?= $plan_cost;?></span><input type="hidden" class="price" value="<?= $plan_cost;?>" name="price"/>			
								</td>
								<td align="center" class="product-quantity">
									<div class="quantity buttons_added txtCal">
										<input id="cur_qty" type="number" step="1" min="0" name="qty" value="1" title="Qty" class="input-text qty text">
