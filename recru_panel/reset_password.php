...
if ($_GET['token']) {
  $user = $recruit_object->get_user_reset_token($_GET['token']);
  if (!$user) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['recaptcha_response'])) {
  ...
  if ($recaptcha->score >= 0.5) {
    ...
    if ($password !== $confirm_password) {
      ...
      $result = $recruit_object->change_user_password($user['recruiter_code'], $password);
      if ($result) {
        $message_success =  "<b>Congratulations!</b><br>Password Reset Successful. ";
        $message_success .= '<br>You can now <a href= ' . SITE_URL . '/recru_panel/login>Login</a> with your new password OR <br> <a href= ' . SITE_URL . '/recru_panel/login>Click Here</a>';
      } else {
