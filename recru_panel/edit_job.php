...
if (isset($_POST['update_job']) && !empty($_POST['update_job'])) {
    ...
    if(empty($course_title) || empty($location) || empty($description)) {
        ...
        $result = $zenta_operation->update_job($job_title, $recruiter_code, $email, $phone, $gallery, $job_company,  $location_id, $country_id, $startDate, $endDate, $category_id, $sub_category_id, $jobstype, $joblevel, $jobsector, $description);
        if($result) {
