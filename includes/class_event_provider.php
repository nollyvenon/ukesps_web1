...
class EventProviderUser
{
    ...
    public function update_event_provider($event_prov_code = NULL, $first_name = NULL, $last_name = NULL, $country = NULL, $state = NULL, $phone = NULL, $billing_address_1 = NULL, $billing_company = NULL, $billing_city = NULL)
    {
        global $db_handle;

        $query = "UPDATE event_providers set first_name='$first_name', last_name='$last_name', billing_country='$country', billing_state='$state', phone='$phone', billing_address_1='$billing_address_1', billing_company='$billing_company', billing_city='$billing_city' WHERE event_prov_code='$event_prov_code'";
...
www.ukesps.com";
    ...
    public function forgot_password($email)
    {
        ...
        $query = "UPDATE event_providers set reset_token = '$gen_pass' where email = '$email'";
        $db_handle->runQuery($query);
        $to = $email;
        $headers = "MIME-Version: 1.0" . "\r\n";
        $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
        ...
        $message = "Folow this link to reset your password : <b><a href='" . SITE_URL . "/events/reset_password?token=$gen_pass'>" . SITE_URL . "/events/reset_password?token=$gen_pass</a> OR <a href='" . SITE_URL . "/events/reset_password?token=$gen_pass'>Click Here</a> </b><br><br>";
    ...
    public function get_user_reset_token($reset_token)
    {
        ...
        $query = "SELECT * FROM event_providers where reset_token = '$reset_token'";
        $result = $db_handle->runQuery($query);
        $fetched_data = $db_handle->fetchAssoc($result);
        return $fetched_data[0];
    }

    public function change_user_password($event_prov_code, $new_password)
    {

        global $db_handle;
        $pass_salt = generateHash($new_password);
        $query = "UPDATE event_providers SET password = '$new_password', pass_salt='$pass_salt', reset_token='' WHERE event_prov_code = '$event_prov_code' LIMIT 1";
        $db_handle->runQuery($query);
    ...
    public function get_cart_info($sid = NULL)
    {
        ...
        $query = "SELECT * FROM event_provider_plan_orders WHERE session_id = '$sid' OR uid='$sid' ORDER BY id ASC";
        $result = $db_handle->runQuery($query);
    ...
    public function get_cart_total($sid = NULL)
    {
        ...
        $query = "SELECT total_price FROM event_provider_plan_orders WHERE session_id = '$sid' OR uid='$sid' ORDER BY id ASC";
        $result = $db_handle->runQuery($query);
    ...
    public function get_order_items($sid = NULL)
    {
        ...
        $query = "SELECT * FROM event_provider_plan_orderitems WHERE session_id = '$sid' ORDER BY id ASC";
        $result = $db_handle->runQuery($query);
    ...
    public function past_payment_det($id)
    {
        global $db_handle;
