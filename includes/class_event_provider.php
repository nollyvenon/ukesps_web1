...
class EventProviderUser {
    ...
    public function authenticate($username = "", $password = "") {
        ...
        if($db_handle->numOfRows($result) == 1) {
            ...
            $query = "SELECT event_prov_code, email, first_name, last_name, active, status FROM event_providers WHERE (event_prov_code='" . $username. "' or email='" . $username. "' or username='" . $username. "') LIMIT 1";
            $result = $db_handle->runQuery($query);
    ...
    public function add_new_event_provider($first_name=NULL, $last_name=NULL, $phone=NULL, $email, $username='', $password, $billing_company,$billing_address_1, $billing_address_2,$billing_city, $billing_state, $billing_country) {
        ...
        if($db_handle->numRows("SELECT event_prov_code FROM event_providers WHERE event_prov_code = '$event_prov_code'") > 0) { goto event_prov_code; };
