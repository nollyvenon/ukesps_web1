...
www.ukesps.com";
    ...
    {
        global $db_handle;
        //check whether company_name already exist generated by rand_string is already existing
        if ($db_handle->numRows("SELECT recruiter_code FROM job_companies WHERE recruiter_code = '$recruiter_code'") > 0) {
            return true;
        } else {
            $query = "INSERT INTO job_companies (recruiter_code, company_name, company_info, company_img) VALUES ('$recruiter_code', '$company_name', '$company_info', '$company_logo')";
            return $db_handle->runQuery($query);
        }
    }

    public function update_recruiter($recruiter_code = NULL, $first_name = NULL, $last_name = NULL, $country = NULL, $state = NULL, $phone = NULL, $billing_address_1 = NULL, $billing_company = NULL, $billing_city = NULL)
    {
        global $db_handle;

        $query = "UPDATE recruiters set first_name='$first_name', last_name='$last_name', billing_country='$country', billing_state='$state', phone='$phone', billing_address_1='$billing_address_1', billing_company='$billing_company', billing_city='$billing_city' WHERE recruiter_code='$recruiter_code'";
        return $db_handle->runQuery($query);
    }
