...

{
    ...
    protected $password;
    protected $port;
    protected $sendto_email;
    protected $sendto_name;
    protected $sendfrom_email;
    protected $sendfrom_name;
    ...
    public function __construct()
    {
        ...
        $this->host = $fetched_data[0]['MailHost'];
        $this->username = $fetched_data[0]['MailUser'];
        $this->smtp_auth = true;
        $this->password = $fetched_data[0]['MailPassword'];
        $this->port = $fetched_data[0]['MailPort'];
        $this->sendto_email = $fetched_data[0]['MailFromEmail'];
        $this->sendto_name = $fetched_data[0]['MailFromName'];
        $this->sendfrom_email = $fetched_data[0]['MailReplyToEmail'];
        $this->sendfrom_name = $fetched_data[0]['MailReplyToName'];
    }
    // function to send SMTP emails
    public function send_user_email($subject, $message, $send_from_email, $send_from_name)
    {
        //PHPMailer Object
        $mail = new PHPMailer;
        $mail->IsSMTP();
        ...
        $mail->SMTPAuth = $this->smtp_auth;     // turn on SMTP authentication
        $mail->Port = $this->port;
        $mail->SMTPSecure = 'ssl';
        ...
        if (isset($send_from_name) && !empty($send_from_name)) {
            $mail->FromName = $send_from_name;

            //Address to which recipient will reply
            $mail->addReplyTo($send_from_email, $send_from_name);
        } else {
            $mail->FromName = 'Contact';

            //Address to which recipient will reply
            $mail->addReplyTo($send_from_email, 'Contact');
        }
        ...
        //To address and name
        $mail->clearAddresses();
        $mail->addAddress($this->sendto_email, $this->sendto_name);
    ...
    // function to send SMTP emails
    public function send_email($subject, $message, $sendto_email, $sendto_name)
    {
        ...
        //From email address and name
        $mail->From = $this->sendfrom_email;
        $mail->FromName = $this->sendfrom_name;
        //Address to which recipient will reply
        $mail->addReplyTo($this->sendfrom_email, $this->sendfrom_name);
        //Send HTML or Plain Text email
        $mail->isHTML(true);
    ...
    public function schedule_campaign($campaign_email_id, $campaign_type)
    {
        ...
        $recipient_query = $db_handle->sanitizePost($client_group);
