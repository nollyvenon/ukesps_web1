...
class CoursProvUser
{
    ...
    public function update_course_provider($course_prov_code = NULL, $first_name = NULL, $last_name = NULL, $country = NULL, $billing_state = NULL, $phone = NULL, $mailingaddress = NULL, $company_name = NULL, $about_me = NULL, $billing_address_1 = NULL)
    {
        global $db_handle;


        $query = "UPDATE course_providers set first_name='$first_name', last_name='$last_name', country='$country', billing_state='$billing_state', phone='$phone', billing_address_1='$billing_address_1', company_name='$company_name', mailingaddress='$mailingaddress', about_me='$about_me' WHERE couprov_code='$course_prov_code'";
    ...
    // Add a new course_providers profile
    public function add_new_course_provider($first_name = NULL, $last_name = NULL, $phone = NULL, $email = NULL, $password = NULL, $company_name = NULL, $contact = NULL, $address = NULL)
    {
        global $db_handle;
        ...
        if ($db_handle->numRows("SELECT couprov_code FROM course_providers WHERE couprov_code = '$couprov_code'") > 0) {
            goto couprov_code;
        };
        ...
        $query = "INSERT INTO course_providers (couprov_code, email, pass_salt, password, first_name, last_name, status, phone, company_name, billing_address_1, mailingaddress) VALUES ('$couprov_code',  '$email', '$pass_salt', '$password', '$first_name', '$last_name', '1', '$phone', '$company_name', '$address', '$contact')";
...
www.ukesps.com";
    ...
    public function forgot_password($email)
    {
        ...
        $query = "UPDATE course_providers set reset_token = '$gen_pass' where email = '$email'";
        $db_handle->runQuery($query);
        $to = $email;
        $headers = "MIME-Version: 1.0" . "\r\n";
        $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
        ...
        $message = "Folow this link to reset your password : <b><a href='" . SITE_URL . "/course_panel/reset_password?token=$gen_pass'>" . SITE_URL . "/course_panel/reset_password?token=$gen_pass</a> OR <a href='" . SITE_URL . "/course_panel/reset_password?token=$gen_pass'>Click Here</a> </b><br><br>";
    ...
    public function get_user_reset_token($reset_token)
    {
        ...
        $query = "SELECT * FROM course_providers where reset_token = '$reset_token'";
        $result = $db_handle->runQuery($query);
        $fetched_data = $db_handle->fetchAssoc($result);
        return $fetched_data[0];
    }

    public function change_user_password($couprov_code, $new_password)
    {

        global $db_handle;
        $pass_salt = generateHash($new_password);
        $query = "UPDATE course_providers SET password = '$new_password', pass_salt='$pass_salt', reset_token='' WHERE couprov_code = '$couprov_code' LIMIT 1";
        $db_handle->runQuery($query);
    ...
    public function add_to_order($total_price = NULL, $total_qty = NULL, $unique = NULL, $paymentmode = NULL)
    {
        ...
        $query = "INSERT INTO course_provider_plan_orders (session_id, total_price, total_qty, orderstatus, paymentmode) VALUES
    ...
    public function get_cart_info($sid = NULL)
    {
        ...
        $query = "SELECT * FROM course_provider_plan_orders WHERE session_id = '$sid' OR uid='$sid' ORDER BY id ASC";
        $result = $db_handle->runQuery($query);
    ...
    public function get_cart_total($sid = NULL)
    {
        ...
        $query = "SELECT total_price FROM course_provider_plan_orders WHERE session_id = '$sid' OR uid='$sid' ORDER BY id ASC";
        $result = $db_handle->runQuery($query);
    ...
    public function get_order_items($sid = NULL)
    {
        ...
        $query = "SELECT * FROM course_provider_plan_orderitems WHERE session_id = '$sid' OR orderid = '$sid' ORDER BY id ASC";
        $result = $db_handle->runQuery($query);
