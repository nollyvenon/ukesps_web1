...
if ($_POST['proceed']){
    ...
    $OrderID = $clientOperation->add_to_order($total_price, $total_qty, $unique, $user_code);
    $_SESSION['OrderID'] = $OrderID;
    $qty = $_POST['qty'];        
    $price = $_POST['price'];
    foreach($qty as $a => $b){
        $course_id = $_SESSION['cart'][$a];
        //$planID = $_POST['planID'];        
        $qty = intval($_POST['qty'][$a]);        
        $price = $_POST['price'][$a];
        $clientOperation->add_to_cart($course_id, $price, $qty, $OrderID, $unique);
			...
			<div id="content" role="main">
				...
				<form action="#" method="post">
					<table id="tblProducts" class="shop_table cart">
						...
						<tbody>	
                            <?php
                            $total_amount =0; print_r($_SESSION['cart']);
