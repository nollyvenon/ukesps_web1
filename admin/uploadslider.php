   ...
   /* foreach($_POST as $key => $value) {
    ...
    if(empty($gallery) || empty($title)) {
		...
		$count = 0;
				...
				if(file_exists("../img/slides/$newfilename")) {
					goto new_name;
				}
		
				$moved = move_uploaded_file($tmp_name, "../img/slides/$newfilename");
				$_FILES['gallery']['name'] = $newfilename;

		$slider = $_FILES['pictures']['name'][0];
		$result = $bio_operation->uploadslider($title, $newfilename, $excerpts);
			if($result) {
				 $message_success = "Slider was successfully uploaded";
			} else {
				$message_error = "slider was not successfully uploaded.";
			}
		}
}
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            <label for="title">
                                Title</label>
                           <div class="input-group"> <input name="title" type="text" id="title" value="<?php echo $title; ?>" size="73" /></div>
                        </div>
                    ...
                    <div class="col-md-12">
                             ...
                             <input name="addcontent" type="submit" class="btn btn-success " id="addcontent" value="Add Slider" />
