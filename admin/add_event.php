...
if (isset($_POST['add_event']) && !empty($_POST['add_event'])) {
    ...
    if(empty($event_title) || empty($postedValue) || empty($page_name)) {
        $message_error = "Please fill all the fields and try again.";
    } else {
        $result = $zentabooks_operation->add_event($event_title, $startDate, $endDate, $location, $summary, $content);
        if($result) {
...
$page_location = $zentabooks_operation->get_page_location();
$page_name = $zentabooks_operation->get_page_name();
