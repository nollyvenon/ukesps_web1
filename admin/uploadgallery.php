...
if (isset($_POST['addcontent']) && !empty($_POST['addcontent'])) {
   ...
   /* foreach($_POST as $key => $value) {
    ...
    if(empty($_FILES['gallery']) || empty($title)) {
		...
		$count = 0;
			    echo $tmp_name = $_FILES["gallery"]["tmp_name"];
				...
				if(file_exists("../img/gallery/$newfilename")) {
					goto new_name;
				}
				$moved = move_uploaded_file($tmp_name, "../img/gallery/$newfilename");
				//$_FILES['gallery']['name'] = $newfilename;

		//$slider = $_FILES['pictures']['name'][0];
		$result = $bio_operation->uploadgallery($title, $newfilename,$excerpts);
			if($result) {
				 $message_success = "Image was successfully uploaded";
			} else {
				$message_error = "Image was not successfully uploaded.";
			}
		}
}
...
<?php include('../header.php');?>			<div class="bg-wrapper aout-page">
                ...
                <div class="row">
                    ...
                    <div class="form-group">
                        <div class="form-group">
                            <label for="content">
                                Content(Recomm size: 660px X 440px)</label>
                    ...
                    <div class="col-md-12">
                             ...
                             <input name="addcontent" type="submit" class="btn btn-success " id="addcontent" value="Add Gallery" />
