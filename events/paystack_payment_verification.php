...
// 		$payment = $event_prov_object->paystack_payment($event_prov_code, $_GET['reference'],$_GET['trxref'], '1', $amount, $_SESSION['couprov_email'],$_SESSION['unique_id'], $payment_category, $currency);
//         //redirect to a thank you page
//         header('Location: paystack_thank_you.php');
//     } else {
// 		$payment = $event_prov_object->paystack_payment($event_prov_code, $_GET['reference'],$_GET['trxref'], '2', $amount, $_SESSION['couprov_email'],$_SESSION['unique_id'], $payment_category, $currency);
...
if ('success' == $tranx->data->status) {
	...
	// die();
	$payment = $event_prov_object->paystack_payment($event_prov_code, $_GET['reference'], $_GET['trxref'], '1', $amount, $_SESSION['event_prov_email'], $_SESSION['unique_id'], $payment_category, $currency, $_SESSION['plan_id']);

	//redirect to a thank you page
	header('Location: paystack_thank_you');
} else {
	$payment = $event_prov_object->paystack_payment($event_prov_code, $_GET['reference'], $_GET['trxref'], '2', $amount, $_SESSION['event_prov_email'], $_SESSION['unique_id'], $payment_category, $currency);
	//mysql_query("DELETE FROM carrrt WHERE code='$unique'");
	header('Location: paystack_error');
}
