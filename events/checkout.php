...
$cart_info = $event_prov_object->get_cart_info($ssid);
...
if ($_POST['create_account']) {
	...
	if ($db_handle->numRows("SELECT username FROM event_providers WHERE username = '$account_username'") > 0) {
		$message_error .= "Username already exists in our database.";
		goto exitpoint;
	};

	$event_providers = $event_prov_object->add_new_event_provider($billing_first_name, $billing_last_name, $billing_phone, $billing_email, $account_username, $account_password, $billing_company, $billing_address_1, $billing_address_2, $billing_city, $billing_state, $billing_country);
...
if ($_POST['login']) {
	...
	if ($found_client) {
		echo $event_prov_code = $found_client[0]['event_prov_code'];
		if ($event_prov_object->event_provider_is_active($event_prov_code)) {
			$found_client = $found_client[0];
			$session_event_prov->login($found_client);
			//redirect_to("checkout");
					...
					<div class="col-lg-6 col-md-6">
						...
						<form action="#" method="post">
							<div class="woocommerce-billing-fields">
								...
								<div class="clear"></div>
								<?php if ($session_event_prov->is_logged_in()) { ?>
							...
							<table>
								<tbody>
									...
									<tr class="cart-subtotal">
										<th>Cart Subtotal</th>
										<td><span class="amount"><?= $cart_info['paymentmode'] . " " . $cart_total; ?></span></td>
									</tr>
									<tr class="shipping">
