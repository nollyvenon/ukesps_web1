...
if ($_GET['token']) {
  $user = $event_prov_object->get_user_reset_token($_GET['token']);
  if (!$user) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['recaptcha_response'])) {
  ...
  if ($recaptcha->score >= 0.5) {
    ...
    if ($password !== $confirm_password) {
      ...
      $result = $event_prov_object->change_user_password($user['event_prov_code'], $password);
      if ($result) {
        $message_success =  "<b>Congratulations!</b><br>Password Reset Successful. ";
        $message_success .= '<br>You can now <a href= ' . SITE_URL . '/events/login>Login</a> with your new password OR <br> <a href= ' . SITE_URL . '/events/login>Click Here</a>';
      } else {
