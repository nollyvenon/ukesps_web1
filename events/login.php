...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['recaptcha_response'])) {
	...
	if ($recaptcha->score >= 0.5) {
		...
		$found_client = $event_prov_object->authenticate($username, $password);
		...
		if ($found_client) {
			...
			if ($event_prov_object->event_provider_is_active($event_prov_code)) {
				...
				redirect_to("my-account");
				// if (!get_cart_info($unique)) { //check if there's an active uniqueID of this session in the order table, if yes redirect to payment page 
				// 	redirect_to("my-account");
				// } else {
	...
	<section class="fullwidth-background bg-2">
		<div class="grid-row">
			<div class="login-block">
				...
				<div class="login-or">
					<hr class="hr-or">
					<span class="span-or">Events Panel Login</span>
					<hr class="hr-or">
				</div>
