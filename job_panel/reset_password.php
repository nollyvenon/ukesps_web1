...
if ($_GET['token']) {
  $user = $jobsk_operation->get_user_reset_token($_GET['token']);
  if (!$user) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['recaptcha_response'])) {
  ...
  if ($recaptcha->score >= 0.5) {
    ...
    if ($password !== $confirm_password) {
      ...
      $result = $jobsk_operation->change_user_password($user['seeker_code'], $password);
      if ($result) {
        $message_success =  "<b>Congratulations!</b><br>Password Reset Successful. ";
        $message_success .= '<br>You can now <a href= ' . SITE_URL . '/job_panel/login>Login</a> with your new password OR <br> <a href= ' . SITE_URL . '/job_panel/login>Click Here</a>';
      } else {
