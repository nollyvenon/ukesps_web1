      ...
      <?php foreach ($user_list as $row) :  ?>
        <tr>
          ...
          <td data-title="Action">
            <?php if ($row['status'] == 1) : ?>
              <a class="btn btn-border green" href="<?= SITE_URL ?>/xadmin/users?deact=<?php echo $row['id'] ?>"><span> <i class="fa fa-edit"></i></span>Deactivate</a>
            <?php else : ?>
              <a class="btn btn-border green" href="<?= SITE_URL ?>/xadmin/users?act=<?php echo $row['id'] ?>"><span> <i class="fa fa-edit"></i></span>Activate</a>
            <?php endif ?>
            <a href="<?= SITE_URL ?>/xadmin/users?did=<?php echo $row['id'] ?>" class="po" title="Delete User" data-placement="top"><i class='fa fa-trash-o'></i> Delete</a>
          </td>
        </tr>
      <?php endforeach ?>
    <?php endif ?>
