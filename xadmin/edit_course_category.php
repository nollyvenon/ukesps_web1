...
$course_cat_details = $zenta_operation->get_course_category_by_id($hiss);
extract($course_cat_details);
if (isset($_POST['edit_course_category']) && !empty($_POST['edit_course_category'])) {
    $course_category = $db_handle->sanitizePost($_POST['course_category']);
    $token = $_POST['token'];
    if (empty($course_category)) {
        $message_error = "Please fill all the fields and try again.";
    } else if ($course_category === $course_cat_details['category_name']) {
        $message_error = "Please Modify name of category and try again";
    } else {
        if ($_FILES['gallery']['name'] != NULL || !empty($_FILES['gallery']['name'])) {
            $uploaddir = "../img/course_category/";
            ...
            $gallery = $course_cat_details['course_cat_img'];
        }
        $result = $zenta_operation->edit_course_category($token, $course_category, $gallery, $admin_id);

        if ($result) {
            $message_success = "Course category was updated successfully.";
        } else {
            $message_error = "Course category was not updated successfully.";
        }
    }
}
                                                ...
                                                <div class="card">
                                                    ...
                                                    <div class="card-block">
                                                        <div class="dt-responsive table-responsive">
                                                            <?php include_once('views/edit_course_category.php'); ?>
                                                        </div>
                                                    </div>
                                                </div>
