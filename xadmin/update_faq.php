...
if (isset($_POST['update_faq']) && !empty($_POST['update_faq'])) {
  $question = $db_handle->sanitizePost($_POST['question']);
  $answer = $db_handle->sanitizePost($_POST['answer']);
  $updated_at = Date("now");
  if (empty($question) || empty($answer)) {
    $message_error = "Please fill all the fields and try again.";
  } else {
    $result = $zenta_operation->update_faq($page_tit, $question, $answer, $updated_at);
    if ($result) {
      $message_success = "Faq was updated successfully.";
      header("Location:manage_faqs.php");
    } else {
      $message_error = "Faq was not updated successfully.";
    }
  }
}
$content = $zenta_operation->get_faqs($page_tit);
extract($content[0]);
?>
<!DOCTYPE html>
<html lang="en">
                        ...
                        <div class="card">
                          ...
                          <div class="card-block">
                            <div class="dt-responsive table-responsive">
                              <?php include_once('views/update_faq.php'); ?>
                            </div>
                          </div>
                        </div>
