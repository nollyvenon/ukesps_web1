...
if (isset($_POST['search_text']) && strlen($_POST['search_text']) > 3) {
	...
	if ($location_id != "") {
        ...
        WHERE (jbl.location_name LIKE '%$search_text%' OR jbv.job_title LIKE '%$search_text%') AND (jbl.location_id='$location_id' OR jbl.location_name LIKE '$location_id') order by jbv.jobs_id DESC ";
	} else {
		$query = "SELECT jbv.*, jbl.location_id, jbl.location_name FROM jobs jbv
        INNER JOIN job_locations jbl ON jbv.job_location=jbl.location_id
        WHERE (jbl.location_name LIKE '%$search_text%' OR jbv.job_title LIKE '%$search_text%') order by jbv.jobs_id DESC ";
	}
} else {
	if ($location_id != "") {
		$query = "SELECT jbv.*, jbl.location_id, jbl.location_name FROM jobs jbv
        INNER JOIN job_locations jbl ON jbv.job_location=jbl.location_id
        WHERE (jbl.location_id='$location_id' OR jbl.location_name LIKE '$location_id') order by jbv.jobs_id DESC ";
	} else {
		$query = "SELECT jbv.*, jbl.location_id, jbl.location_name FROM jobs jbv
				...
				<main>
					...
					<div class="category-search">
						...
						<form action="" method="post" class="form-horizontal tasi-form" enctype="multipart/form-data">
							...
							<button class="cws-button smaller border-radius alt">Search Job Location</button>
						</form>
					</div>
					<h3>Search for <?php echo $_POST['search_text']; ?> Location</h3>
					<!-- / search -->
					...
					?>
							...
							<div class="category-item list clear-fix">
								...
								<div class="category-info">
									<span class="price">
										<span class="amount">
											<?= $SiteCurrency; ?><?= formatMoney($amount_per_period, 2); ?>
										</span>
