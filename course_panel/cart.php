...
if ($_POST['proceed']){
    ...
    foreach($qty as $a => $b){
        ...
        $price = $_POST['price'][$a];
        $course_prov_object->add_to_cart($plan_id, $price, $qty, $OrderID, $unique);
...
if ($_POST['delete_cart_action']){
	$unique = $_SESSION['unique'];
    $course_prov_object->delete_cart($unique);
    unset($_SESSION['cart']);
    ...
    if ($_POST['deleteitem']){ 
      ...
      $data1 = $course_prov_object->delete_cart_item($unique, $plan_id);
        if ($data1){
						...
						<tbody>	
                            ...
                            for($i=0; $i<$max; $i++) { 
								...
								if ($_SESSION['payment_category'] == '1'){ //recruitment
                                	$recru_detail = $course_prov_object->recruiting_plan_detail_by_id($plan_id);
                                	extract( $recru_detail);
								}elseif($_SESSION['payment_category'] == '2'){//cv search
									$cv_detail = $course_prov_object->cvsearch_detail_by_id($plan_id);
                                	extract( $cv_detail);
								}
                            ?>
							<tr  class="cart_item">
			...
			<div class="grid-row">
				...
				<div class="owl-carousel testimonials-carousel">
					...
					<div class="gallery-item">
						<div class="quote-avatar-author clear-fix"><img src="img/testimonials/<?=$row['testi_image'];?>" data-at2x="img/testimonials/<?=$row['testi_image'];?>" alt=""><div class="author-info"><?=$row['testi_name'];?><br><span><?=$row['testi_designation'];?></span></div></div>
						<?=$row['testi_content'];?>
					</div>
					<?php }} ?>
				</div>
