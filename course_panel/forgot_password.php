...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['recaptcha_response'])) {
	...
	if ($recaptcha->score >= 0.5) {
		...
		if ($db_handle->numRows("SELECT couprov_code FROM course_providers WHERE email = '$email'") > 0) {
			$result = $course_prov_object->forgot_password($email);
			if ($result) {
			...
			$message_error .= 'No Course Provider with such email';
		};
	} else {
		// spam submission
		// show error message
