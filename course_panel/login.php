...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['recaptcha_response'])) {
	...
	if ($recaptcha->score >= 0.5) {
		...
		$found_client = $course_prov_object->authenticate($username, $password);

		if ($found_client) {
			echo  $course_prov_code = $found_client[0]['couprov_code'];
			if ($course_prov_object->course_provider_is_active($course_prov_code)) {
				...
				if (!$course_prov_object->get_cart_info($unique)) { //check if there's an active uniqueID of this session in the order table, if yes redirect to payment page 
					redirect_to("index");
				} else {
	...
	<section class="fullwidth-background bg-2">
		<div class="grid-row">
			<div class="login-block">
				...
				<div class="login-or">
					<hr class="hr-or">
					<span class="span-or">Course Panel Login</span>
					<hr class="hr-or">
				</div>
